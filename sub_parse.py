...
titles = [  'euler', 'indiv-euler', 'all-euler', 'indiv_silh', 'all-silh', \
            'silh-euler', 'contour', '']
...
def remove_trash(content):
    title_names = get_title()
    bad_stuff = ['[1] "--------------------------------"\n']
    new_content = [line.replace('\n', '') for line in content if not line in bad_stuff]
    new_content = ['Test for %s :' % title_names.next()] + new_content
    
    for i in range(len(new_content)):
        if new_content[i] == '[1] ""':
            new_content[i] = 'Test for %s :' % title_names.next()
    ...
    return_content = [line for line in best_content if not line == '']
