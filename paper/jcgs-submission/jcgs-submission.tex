...
%   \caption{Examples of eight proposed test statistics. A Voronoi foam with a percent filament of 0.5 generates the point cloud dataset shown in (a) with persistence diagram (b).
%The remaining sub-figures (c) - (h) correspond to the test statistics: (c) EC - Euler characteristic, (d) SIL - weighted silhouette for $H_0$, (e) IK - Intensity function for $H_0$, (f) WIK - weighted intensity function, (g) PI - persistence image, (h) CORR - correlation function.
%See Section \ref{sec:methods} for more details.}
   \label{fig:examples}
\end{figure}
