...
The simulation study results are displayed in \figref{fig:subLinesUnnormApp}, which shows the median $\log_{10}$ p-values along with a (25\%, 75\%) confidence interval across the 1000 iterations of the proposed test statistics.


From \figref{fig:subLinesUnnormApp}, we see that  EC,  CORR, and SILEC test are the most effective in distinguishing differences between the models.
...
\begin{center}
  \begin{figure}[htp!]
    ...
    \end{subfigure}
    \caption{Results for EC, SIL, IK, WIK, PI, and CORR tests. X-axis represents the true PercFil (10\%, 15\%, 20\%, 25\%, 30\%) of the second population, compared to the null PercFil of 10\%; The vertical axis shows the $\log_{10}$(p-values). The lines plots the median $\log_{10}$ p-value and error bars show the 25th and 75th percentiles of the 1000 iterations. The right column of plots show the ratio of the unstandardized p-value divided by the standardized p-value for each PercFil.
    }
...
Similar to the simulation models from \S \ref{sec:sim_model}, Megaparsec cosmic mass distributions are likewise characterized by intricate multiscale configuration of web-like filaments and voids. We apply the proposed methodology to cosmological simulation data.
